...
async def run_visura(
    ...
    logger.info(
        f"[VISURA] Selezionando tipo catasto: {tipo_catasto} ({'Terreni' if tipo_catasto == 'T' else 'Fabbricati'})"
    )
    try:
    ...
    try:
        ...
        for result_index in range(radio_count):
            ...
            if partita == "Soppressa":
                logger.info(
                    f"[VISURA] Risultato {result_index + 1} ha partita 'Soppressa', saltando estrazione intestati"
                )
...
async def extract_all_sezioni(page: Page, tipo_catasto: str = "T", max_province: int = 200) -> list:
    ...
    try:
        logger.info(
            f"[SEZIONI] Iniziando estrazione sezioni per tipo catasto: {tipo_catasto} (max {max_province} province)"
        )
...
async def run_visura_immobile(
    ...
    if sezione:
        ...
        if not available_sections:
            logger.info(
                f"[VISURA_IMMOBILE] Nessuna sezione disponibile per il comune '{comune}', saltando selezione sezione"
            )
        else:
